...
namespace Datos.SISU
{
    public class DIngresoServUrb
    {
                ...
                string desc = resp.respValidarDireccion.estadoOperacion.descripcionEstado;
                    ...
                    foreach (DireccionType dtype in resp.respValidarDireccion.listaDirecciones)
                    {
                        ...
                        else
                        {
                            ...
                            if (dtype.numeracion == "") { dtype.numeracion = "0"; Direccion.Numeracion = Convert.ToInt32(dtype.numeracion); } else { Direccion.Numeracion = Convert.ToInt32(dtype.numeracion); }
        ...
        public List<Valorizacion> ObtieneValorizacionServicios(string CodCoberturaOri, string CodCoberturaDes, int Producto, decimal Peso, int Alto, int Ancho, int Largo)
        {
            ...
            try
            {
                dr = this.BDExp_Corp.ExecuteReader("pr_s_ValorizaCourier_SU"
