...
namespace Datos.SISU
{
    public class DConsultaHorario
    {
        ...
        public Horario ConsultaHorarioPorNumero(int NroTCC)
        {
            ...
            try
            {
                dr = this.BDExp_Corp.ExecuteReader("AI_BuscaHora", NroTCC);
                if (dr.Read())
                {

                    Hr = new Horario();                    
                    if (dr["Hora"] != DBNull.Value)
                        Hr.Hora = Converts.ConvertToInt(dr["Hora"]);

                    if (dr["Minutos"] != DBNull.Value)
                        Hr.Minutos = Converts.ConvertToInt(dr["Minutos"]);
