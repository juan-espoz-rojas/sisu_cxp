...
namespace Infraestructura.WebConstrols
{
    ...
    public class FailedValidationsControl : WebControl 
    {
        ...
        public void ShowException(Exception ex)
        {
            ...
            if (CssPath == "/Views/TCCs.aspx")
            {
                
                this.Exception = new Exception("TCC no cumple con configuración para servicio de Same Day Delivery");
            }

            if (CssPath != "/Views/IngresoServicioUrb.aspx" )
            {
                if (CssPath != "/Views/TCCs.aspx") 
                {
